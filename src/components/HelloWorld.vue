...
<template>
  <section class="flex w-full max-w-md flex-col gap-6 rounded-2xl border p-8">
    ...
    <form class="flex gap-2" @submit.prevent="submit">
      <input
        ...
        type="text"
        placeholder="输入你的名字"
        class="flex-1 rounded-md border bg-background px-3 py-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"
      />
      <button
        type="submit"
        :disabled="!canSubmit"
        class="inline-flex shrink-0 items-center gap-1.5 rounded-md bg-accent px-4 py-2 text-sm font-medium whitespace-nowrap text-accent-foreground hover:opacity-80 disabled:cursor-not-allowed disabled:opacity-50"
      >
    ...
    <p v-else-if="errorMessage" class="text-sm text-foreground">{{ errorMessage }}</p>
  </section>
</template>
